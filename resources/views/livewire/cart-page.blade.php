                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center">
                                            <button wire:click="decreaseQty({{ $item['product_id'] }})"
                                                class="border rounded-md py-2 px-2 mr-2">-</button>
                                            <span class="text-center w-8">{{ $item['quantity'] }} </span>
                                            <button wire:click="increaseQty({{ $item['product_id'] }})"
                                                class="border rounded-md py-2 px-2 ml-2">+</button>
                                        </div>
                                    </td>
                                    <td class="py-4">{{ Number::currency($item['total_amount'], 'IDR') }} </td>
                                    <td>
                                        <button wire:click="removeItem({{ $item['product_id'] }})"
                                            class="bg-slate-300 border-2 border-slate-400 rounded-lg px-2 py-1 hover:bg-red-500 hover:text-white hover:border-red-700">
            ...
            <div class="md:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span class="font-semibold">{{ Number::currency($grand_total + $tax_amount, 'IDR') }}</span>
                    </div>
                    @if ($cart_items)
                        <a href="/checkout"
